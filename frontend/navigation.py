...
class Navigation:
    ...
    def show_section(self, section_id):
        ...
        if section_id:
            ...
            if target:
                ...
                if section_id == "admin" and self.user_login.current_user_info and self.user_login.current_user_info["authorizations"].get("is_admin"):
                    self.user_admin.load_users()
                # Show CSV upload form when games section is shown and user is contributor
                if section_id == "games" and self.user_login.current_user_info and self.user_login.current_user_info["authorizations"].get("is_contributor"):
                    self.show_add_game_form()
                    self.show_csv_upload_form()
                # Load games when games section is shown
                if section_id == "games" and self.user_login.current_user_info and self.user_login.current_user_info["authorizations"].get("is_viewer"):
                    self.show_games()
    ...
    def handle_navigation(self):
        ...
        if self.logged_in_callback():
            login_link[0].text = self.user_login.current_user_info.get("username", "unknown user")
            logout_container[0].style.display = "block"
            login_container[0].style.display = "none"
            if self.user_login.current_user_info and self.user_login.current_user_info["authorizations"].get("is_admin"):
